...
export default function ProductDetailsClient() {
  ...
  return (
    <main className="p-20 max-[550px]:p-5 max-[639px]:mt-[4rem]">
      ...
      <section className="bg-white text-black mt-10 mb-16 items-center rounded-2xl border-1 border-black">
        <article className="flex flex-wrap max-[1000px]:flex-col max-[800px]:items-center p-10 max-[400px]:px-2">
          ...
          <div className="w-2/3">
            ...
            <Button startContent={<FontAwesomeIcon icon={faCartArrowDown}/>} className={`w-1/2 max-[1000px]:w-2/3 max-[650px]:w-full text-xl font-semibold rounded-2xl my-5 ${isOnCart ? "bg-green-600" : "bg-gray-300"}`} onPress={handleAddToCart} isDisabled={isOnCart}>
              {isOnCart ? "Agregado" : "Agregar"}
            </Button>
            <h4 className={`text-lg font-semibold underline ${product.stock > 15 ? "text-green-700" : product.stock > 10 ? "text-orange-500" : "text-red-600"}`}>
              Stock {product.stock > 15 ? "alto" : product.stock > 10 ? "medio" : "bajo"}
            </h4>
